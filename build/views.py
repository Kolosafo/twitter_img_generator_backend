...
@api_view(['POST'])
def realDownloadsCount(request):
    inialtCount = RealVoteCount.objects.get(pk=1)
    inialtCount.count += 1
    inialtCount.save()
    return JsonResponse("Added", safe=False, status=200)

@api_view(['GET'])
def getRealCount(request):
    real_votes = RealVoteCount.objects.get(pk=1)
    returnObj = {
        "count": real_votes.count
    }
    return JsonResponse(returnObj, safe=False, status=200)
